...
func findLengthOfArrays(nums1 []int, nums2 []int) int {
	l1, l2 := len(nums1), len(nums2)
	return l1 + l2
}

func findMedianSortedArraysb(nums1 []int, nums2 []int) float64 {
	length := findLengthOfArrays(nums1, nums2)
	if length%2 == 1 {
		kthFormula := length/2+1
		return float64(findKthNum(nums1, nums2, kthFormula))
	} else {
...
func findKthNumB(numArray []int, numArrayTwo []int, k int) (val int) {
